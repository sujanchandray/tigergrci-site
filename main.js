...
(function () {
  ...
  /* Foreground-only mild BGM: never plays in background; restarts from 0 on return. */
  (function initMildBgm() {
    ...
    function playFromStart() {
      ...
      if (!audio.paused && pageIsForeground()) {
        setUi(true);
    ...
    function syncForeground() {
      if (!pageIsForeground()) onLeaveForeground();
    ...
    window.addEventListener("blur", onLeaveForeground);
    window.addEventListener("focus", function () {
      if (pageIsForeground()) onEnterForeground();
      else onLeaveForeground();
    });
    window.addEventListener("pagehide", onLeaveForeground);
    document.addEventListener("freeze", onLeaveForeground);
    ...
    if (pref === "0") return;
    if (reduceMotion && pref !== "1") return;

    function tryStartOrUnlock() {
      if (pref === "1") wantOn = true;
      playFromStart().then(function (ok) {
        if (ok || readPref() === "0") return;
