...
(function () {
  ...
  (function initWorkflowDemo() {
    ...
    /** Hide the evidence token if a browser paints it outside the canvas. */
    function bindTokenCanvasGuard(slide) {
      if (!slide || !archCanvas || slide.getAttribute("data-token-guard") === "1") return;
      const token = slide.querySelector(".flow-doc-token");
      if (!token) return;
      slide.setAttribute("data-token-guard", "1");
      let raf = 0;
      const tick = function () {
        if (!document.contains(slide)) return;
        if (slide.classList.contains("is-active")) {
          const cr = archCanvas.getBoundingClientRect();
          ...
          if (tr.width > 0 && tr.height > 0) {
            const out =
              ...
              tr.top < cr.top - 0.5;
            token.style.visibility = out ? "hidden" : "visible";
          }
        }
        raf = window.requestAnimationFrame(tick);
      };
      raf = window.requestAnimationFrame(tick);
      slide._tokenGuardRaf = raf;
    }
    ...
    function preloadArchSlides() {
      ...
      return Promise.all(jobs).then((results) => {
        ...
        results.forEach((item) => {
          ...
          bindTokenCanvasGuard(item.node);
        });
        archReady = true;
